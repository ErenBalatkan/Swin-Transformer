...
def test(model, transform, config, dataloader):
    model = model.cuda()
    ...
    if ENABLE_WANDB:
        wandb_test_table = wandb.Table(columns=["id", "pred", "gt", "img"])
    ...
    with torch.inference_mode():
        for idx, item in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if ENABLE_WANDB:
                log_imgs = torch.nn.functional.interpolate(raw_img, [128, 128]).permute(0, 2, 3, 1).cpu().numpy()
                for idx, item in enumerate(pred_idx):
                    log_img = log_imgs[idx]
                    wandb_test_table.add_data(img_id[idx], item, label[idx], wandb.Image(log_img))

    wandb.log({
        "table": wandb_test_table
    })
...
if __name__ == '__main__':
    ...
    dataset = LogMealTypesDataset(path, transform, MODE, return_id=True)
    ...
    wandb.init(project="logmeal_drinks", name="Analysis")

    test(model, config, dataloader)
