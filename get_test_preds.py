...
import pandas
from data.cached_image_folder import pil_loader, find_classes
import os
...
class ICCVDataset(Dataset):
    def __init__(self, path, transforms, mode="test"):
        ...
        if mode == "test":
            ...
            label_dirs = os.listdir(self.path)[:10]
            for idx, label_dir in enumerate(label_dirs):
                img_paths = os.listdir(os.path.join(self.path, label_dir))
    ...
    def __getitem__(self, item):
        ...
        img = self.transforms(raw_img)

        return img, int(label) if label is not None else 0, img_path

    def __len__(self):
        return self.len


def reorder_pred_columns_to_iccv_format(preds):
    new_preds = torch.zeros_like(preds)

    for idx, (key, value) in enumerate(oht_index_to_class.items()):
        key_index = int(key)
        new_preds[:, int(value)] = preds[:, key_index]

    return new_preds


transform_post = torch.nn.Sequential(
    K.augmentation.RandomRotation(30),
    # K.augmentation.RandomHorizontalFlip(),
    # K.augmentation.RandomResizedCrop((224, 224), (0.8, 1)),
).cuda()


def test(model, transform, config, enable_wandb, mode):
    path = config.DATA.DATA_PATH
    dataset = ICCVDataset(path, transform, mode)
    ...
    if not LOAD_TS:
        ...
        if OPTIMIZE_INF:
            ...
            torch.jit.save(model, "swin_super.ts")
    ...
    with torch.inference_mode():
        ...
        for idx, item in tqdm(enumerate(dataloader), total=len(dataloader)):
            raw_img, label, img_id = item
            ...
            if not TTA:
                ...
                img = img.repeat(TTA_COUNT, 1, 1, 1)
                img[1:] = transform_post(img[1:])
                out = model(img)
                out[0] *= 2
                out = torch.einsum("bc->c", [out]).unsqueeze(0) / (TTA_COUNT + 1)

            exec_time = time.time() - before

            pred = reorder_pred_columns_to_iccv_format(out)
            pred_idx = torch.argmax(pred, 1).cpu().numpy()

            if label is not None:
                acc1 = metrics.accuracy_score(label, pred_idx)
                acc1_meter.update(acc1, len(pred))
                labels += label
            ...
            log_dict = {
                "Execution Time": exec_time * 1000,
                "Allocated Memory": allocated_memory / (1024 * 1024),
                "step": idx,
                "acc1": acc1_meter.avg
            }
    ...
    wandb.log({
        "test_table": wandb_test_table
    })

    preds_idx = torch.argmax(preds, 1).cpu().numpy()

    if len(labels) == len(preds_idx):
        acc = metrics.accuracy_score(labels, preds_idx)
        acc_balanced = metrics.balanced_accuracy_score(labels, preds_idx)
        print(f"Acc: {acc}  |  Acc Balanced: {acc_balanced}")

    numpy_preds_file_path = f"numpy_preds_{mode}"
    if os.path.exists(f"{numpy_preds_file_path}.npz"):
        os.remove(f"{numpy_preds_file_path}.npz")
    np.savez_compressed(numpy_preds_file_path, scores_array=preds.cpu().numpy().astype(np.float16),
                        file_array=img_ids)
    ...
    csv_file_path = f"iccv_preds_{mode}.csv"
...
if __name__ == '__main__':
    ...
    if not LOAD_TS:
        ...
        model = torch.jit.load("swin_super.ts", "cuda")

    enable_wandb = ENABLE_WANDB
    project_name = "PyTorch Benchmark"
    model_name = f"Swin BS:{BATCH_SIZE}"
    ...
    if enable_wandb:
        name = model_name
        if HALF or SCRIPT or OPTIMIZE_INF:
            name += " +"
        if HALF:
            name += "H"
        if SCRIPT:
    ...
    test(model, transform, config, enable_wandb, MODE)
    # test(model, transform, config, enable_wandb, "val")
    # test(model, transform, config, enable_wandb, "train")
